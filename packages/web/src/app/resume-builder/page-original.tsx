...
export default function ResumeBuilderPage() {
  ...
  const generateSummarySuggestion = async () => {
    if (resumeData.experience.length === 0 && resumeData.skills.every(skill => skill.skills.length === 0)) {
      showError("Cannot generate summary", "Please add your work experience and skills first.");
      return;
    }
    ...
    try {
      ...
      showSuccess("Summary generated!", "AI created a professional summary for you.");
    } catch (error) {
      console.error("Failed to generate summary suggestion:", error);
      showError("Summary generation failed", "Unable to generate AI summary. Please try again.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const generateAchievementSuggestions = async (experienceIndex: number) => {
    try {
      ...
      if (!experience.position || !experience.company) {
        showError("Missing information", "Please enter your position and company name.");
        return;
      }
      ...
      showSuccess("Achievements suggested!", "AI added impactful achievements to your experience.");
    } catch (error) {
      console.error("Failed to generate achievement suggestions:", error);
      showError("Achievement generation failed", "Unable to generate AI suggestions. Please try again.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const exportAsPDF = async () => {
    try {
      ...
      showSuccess("Resume ready for printing!", "Use your browser's print dialog to save as PDF.");
    } catch (error) {
      console.error("Failed to export PDF:", error);
      showError("Export failed", error instanceof Error ? error.message : "Unable to prepare your resume for printing.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchResume = useCallback(async () => {
    ...
    try {
      ...
      showError("Load failed", "Unable to load your resume data. Please refresh the page.");
    } finally {
      setLoading(false);
    }
  }, [user]);
  ...
  const addExperience = () => {
    ...
    showSuccess("Experience added!", "New work experience has been added to your resume.");
  };
  ...
  const duplicateExperience = (index: number) => {
    ...
    showSuccess("Experience duplicated!", "Copy created for easy editing.");
  };
  ...
  const addSuggestedSkill = (categoryIndex: number, skill: string) => {
    ...
    if (!currentSkills.includes(skill)) {
      ...
      showSuccess("Skill added!", `"${skill}" has been added to your skills.`);
    } else {
      showError("Duplicate skill", "This skill is already in your list.");
    }
  };
  ...
  const addProject = () => {
    ...
    showSuccess("Project added!", "New project has been added to showcase your work.");
  };
